...
function main (masterJSON, currentJSON, _resultJSONPath) {
  return Promise.all([masterJSON, currentJSON]).then((resultObjectArrays) => {
    ...
    // TODO: вынести запись в файл из модуля
    writeResult(unicalErrors, _resultJSONPath || resultJSONPath);
    return unicalErrors;
  });
};
    ...
    if (errorsMapMasterEmpty) {
      ...
      errorsMapCurrent.forEach((value, key) => {
        ...
        if (masterHasErrorInFile && !errorsInFileIsIden) {
          lintResultWithUnicalNewMessages = getLintResultWithUnicalNewMessages(value, errorsMapMaster.get(key));
          if (lintResultWithUnicalNewMessages) {
            unicalNewErrors.set(key, lintResultWithUnicalNewMessages);
          }
        } else if (!errorsInFileIsIden && value.messages.length > 0) {
