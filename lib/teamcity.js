...
main.getBuildStatistics = getBuildStatistics;
 ...
 */
function getBuildStatistics (statisticsParameterName) {
  const options = { method: 'GET',
    url: `${creditials.host}/repository/download/${creditials.buildTypeId}/${buildId}:id/statistics`,
    headers:
      {
        'cache-control': 'no-cache',
  ...
  return fetch(options.url, options).then(function (response) {
    return response.json().then(result => {
      let buildStatisticsParameters = statisticsParameterName
        ? utils.filerArrayOfObjectsByPropertyValue(result.property, 'name', statisticsParameterName)
        : result;
