 ...
 */
async function init (_creditials, masterBranchName) {
  let pending = [];
  setCreditials(_creditials);
  let _branches = await getBranches(_creditials.buildTypeId);
  pending.push(setLatestSuccessfullBuildId(encodeURIComponent(masterBranchName || _branches[1].internalName || 'master')));
 ...
 * @return {Object[]|Object} - значение параметра или вся статистика
 */
function getBuildStatistics (statisticsParameterName, _buildId) {
  let buildId = _buildId || buildId;

  const options = { method: 'GET',
    url: `${creditials.host}/app/rest/builds/buildId:${buildId}/statistics`,
    headers:
      {
        'cache-control': 'no-cache',
 ...
 * @returns {Array} объект с набором веток
 */
function getBranches(buildTypeId=creditials.buildTypeId) {
  const url = `${creditials.host}/app/rest/buildTypes/id:${encodeURIComponent(buildTypeId)}/branches?fields=branch(internalName)`,
        fetchOpt = {
          method: 'GET',
          headers:  {
            'Accept': 'application/json',
